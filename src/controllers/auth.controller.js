        ...
        try {
            ...
            if (result) {
                ...
                return res.status(201).json({
                    message: 'User registration has successful, please check your email for activation account',
                    data: result,
                });
            } else {
                throw (500, 'User registration not successful, please try again');
            }
        } catch (error) {
            next(error);
        }
...
const login = async(req, res, next) => {
    try {
        ...
        const token = jwt.sign({ email: user.email, user_id: user.id, role: user.role_id },
            process.env.JWT_SECRET, {
                expiresIn: '2m',
            }
        );

        const refresh = jwt.sign({ email: user.email, user_id: user.id, role: user.role_id },
...
const refreshToken = async(req, res, next) => {
    ...
    const accessToken = jwt.sign({ email: email, user_id: isTokenValid.user_id },
        process.env.JWT_SECRET, {
            expiresIn: '2m',
        }
    );
...
const confirm = async(req, res, next) => {
    ...
    //     return res.status(400).json({
    //         message: 'Your activation token has expired, please resend email activation',
    //     });
    // }
